...
;
; formerly det21_plot.pro
; currently configured to aid with detector comparison 
;
;
;
...
;
;------------------
PRO detxx_plot, XRANGE=x__range, YRANGE=y__range, XTITLE=x__title, YTITLE=y__title, DET=XX, INPUT=ipath, OUTPUT=opath
    ...
    ; flow control
    simulate_fsw = 0
    plot_sim_data = 0		; plot simulated ata
    plot_fsw_data = 0		; plot FSW-derived ata
    plot_raw_data = 1		; plot raw STEIN data (collected via native interface)
    

    ; load event-lists
    
    ; FIRST, from RAW DATA
    data = LOAD_EVENTLIST(ipath + PATH_SEP() + "Am241det" + XX + ".txt")
    ...
    persheet = 4
    ps_output = 1
    name_stem = opath + PATH_SEP() + "det" + XX + "_pixel"
    ; (directory + name) stem for output filenames
        ...
        FOR j=0, persheet-1 DO BEGIN
            ...
            IF (plot_raw_data) THEN BEGIN
             ...
             IF (raw_n GT 0) THEN BEGIN
                ...
                PLOT, fullRange, current_channel, XRANGE=[1.e4,3.e4], XTITLE='ADC value (0-65535)', YTITLE="# Events (per ADC value)", PSYM=3, TITLE="Detector "+ XX +", Pixel # "+ strcompress(det,/REMOVE_ALL);, MIN_VALUE=1, /YLOG
                ADC_values = fullRange
                STEIN_histogram = current_channel
                SAVE, ADC_values, STEIN_histogram, FILENAME=name_stem+strcompress(det, /REMOVE_ALL)+".idlsav"
             ENDIF ELSE BEGIN
                PLOT, [0,1], [0,1], /NODATA, XRANGE=[1.0e4,3.0e4], YRANGE=[0,approx_height], XTITLE='ADC value (0-65535)', YTITLE='# Events (per ADC value)', TITLE="Detector "+ XX +", Pixel # "+strcompress(det, /REMOVE_ALL)
